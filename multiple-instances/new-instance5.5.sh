...
if ! cp ${script_dir}/tomcat5.5/web.xml ${instance_dir}/conf; then
  echo "Unable to create new instance ${host_name} because ${script_dir}/tomcat5.5/web.xml doesn't appear to exist"
  exit 1
fi

if ! sed "s/@HOST_NAME@/${host_name}/g" ${script_dir}/tomcat5.5/server.xml | sed "s/@HOST_NAME_UNDERSCORES@/${host_name_underscores}/g" | sed "s/@PORT@/${port}/g" | sed "s/@JK_PORT@/${jk_port}/g" > ${instance_dir}/conf/server.xml; then
  echo "Unable to create new instance ${host_name} because ${script_dir}/tomcat5.5/server.xml doesn't appear to exist"
  exit 1
fi
